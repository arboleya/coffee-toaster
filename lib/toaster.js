(function() {
    ...
    Config.prototype.create = function(folderpath) {
      ...
      return this.ask(q1.magenta, /.+/, function(src) {
        return _this.ask(q2.magenta, /.+/, function(release) {
          return _this.ask(q3.cyan, /.*/, function(webroot) {
            return _this.write(src, release, webroot);
          });
        });
      });
    };

    Config.prototype.write = function(src, release, webroot) {
  ...
  pkg('toaster.generators').Project = Project = (function(_super) {
    ...
    Project.prototype.create = function(folderpath, name, src, release) {
      ...
      return this.ask(q1.magenta, /.*/, function(src) {
        ...
        return _this.ask(q2.magenta, /.*/, function(release) {
          ...
          return _this.ask(q3.cyan, /.*/, function(webroot) {
            var $release, $src, $webroot;
            if (webroot == null) {
              webroot = null;
            }
            $src = src || "src";
            $release = release || "www/js/app.js";
            if (src === '' && release === '' && webroot === '') {
              $webroot = 'js';
            } else {
              $webroot = webroot || "";
            }
            _this.scaffold(target, $src, $release, $webroot);
            return process.exit();
          });
        });
      });
    ...
    Project.prototype.scaffold = function(target, src, release, webroot) {
      ...
      return new Config(target).write(srcdir, releasefile, webroot);
    };
