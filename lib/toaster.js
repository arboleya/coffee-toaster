(function() {
  var ArrayUtil, Builder, Cli, Config, FnUtil, FsUtil, Project, Question, Script, StringUtil, Toast, Toaster, debug, error, growl, icon_error, icon_warn, interval, log, msgs, pkg, process_msgs, queue_msg, start_worker, stop_worker, toaster, warn,
    __slice = [].slice,
  ...
  pkg('toaster.utils').FnUtil = FnUtil = (function() {
  ...
  pkg('toaster.core').Builder = Builder = (function() {
    ...
    Builder.prototype._is_building = false;

    Builder.prototype._toaster_helper = "__t = ( ns, expose )->\n	curr = null\n	parts = [].concat = ns.split \".\"\n	for part, index in parts\n		if curr == null\n			curr = eval part\n			expose[part] = curr if expose?\n			continue\n		else\n			unless curr[ part ]?\n				curr = curr[ part ] = {}\n				expose[part] = curr if expose?\n			else\n				curr = curr[ part ]\n	curr\n";

    Builder.prototype._include_tmpl = "document.write('<scri'+'pt src=\"%SRC%\"></scr'+'ipt>')";

    function Builder(toaster, toast, cli) {
      ...
      this.vendors = (_ref = this.toast.vendors) != null ? _ref : [];
      this.bare = (_ref1 = this.toast.bare) != null ? _ref1 : this.cli.argv.bare;
      this.packaging = (_ref2 = this.toast.packaging) != null ? _ref2 : this.cli.argv.packaging;
      this.expose = (_ref3 = this.toast.expose) != null ? _ref3 : this.cli.argv.expose;
      this.minify = (_ref4 = this.toast.minify) != null ? _ref4 : this.cli.argv.minify;
      this.exclude = [].concat((_ref5 = this.toast.exclude) != null ? _ref5 : this.cli.argv.exclude);
      this.httpfolder = (_ref6 = this.toast.httpfolder) != null ? _ref6 : '';
      this.release = this.toast.release;
      this.debug = this.toast.debug;
      this.init(function() {
    ...
    Builder.prototype.init = function(after_init) {
      var folder, folder_num, _i, _len, _ref, _results,
        _this = this;
      this.files = this.toaster.files;
      folder_num = this.toast.src_folders.length;
      _ref = this.toast.src_folders;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        folder = _ref[_i];
        _results.push(FsUtil.find(folder.path, "*.coffee", FnUtil.proxy(function(folder, result) {
          var falias, file, fpath, include, item, s, _j, _k, _len1, _len2, _ref1;
          fpath = folder.path;
          falias = folder.alias;
          for (_j = 0, _len1 = result.length; _j < _len1; _j++) {
            ...
            if (include) {
              s = new Script(_this, fpath, file, falias, _this.cli, _this.bare);
              _this.files.push(s);
            }
          }
          if (--folder_num === 0) {
    ...
    Builder.prototype.build = function(after_build) {
      var _this = this;
      if (this._is_building === true) {
        return;
      }
      this._is_building = true;
      return this.build_namespaces(function(namespaces) {
        var ast, contents, f, files, helper, i, include, tmpl, vendors, _i, _len;
        helper = cs.compile(_this._toaster_helper, {
          bare: true
        });
        vendors = _this.merge_vendors();
        contents = [vendors, helper, namespaces, _this.compile()].join('\n');
        if (_this.minify) {
    ...
    Builder.prototype.build_namespaces = function(after_build_namespaces) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (folder.alias != null) {
          ...
          pending++;
          FsUtil.ls_folders(folder.path, function(subfolders) {
            var name, subfolder, _j, _len1;
            for (_j = 0, _len1 = subfolders.length; _j < _len1; _j++) {
              subfolder = subfolders[_j];
              name = subfolder.match(/[^\/]+$/m);
              namespaces += _this.build_namespaces_declaration(name);
            }
            if (--pending === 0) {
              return after_build_namespaces(namespaces);
            }
          });
        }
      }
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(FsUtil.watch_folder(src.path, /.coffee$/, FnUtil.proxy(function(src, info) {
          ...
          switch (info.action) {
            case "created":
              if (info.type === "file") {
                s = new Script(_this, fpath, ipath, falias, _this.cli, _this.bare);
              }
    ...
    Builder.prototype.compile = function() {
      ...
      return cs.compile(output, {
        bare: this.bare
      });
    };

    Builder.prototype.compile_for_debug = function() {
      var absolute_path, file, files, folder_path, index, msg, relative_path, release_path, _i, _len, _ref;
  ...
  pkg('toaster.core').Script = Script = (function() {
    ...
    Script.prototype.getinfo = function() {
      ...
      if (this.raw.match(rgx) != null) {
        if (this.namespace !== "") {
          if (this.builder.packaging && !(this.builder.expose != null)) {
            repl = "$1 __t('" + this.namespace + "').$2$3";
          } else if (this.builder.packaging && (this.builder.expose != null)) {
            repl = "$1 __t('" + this.namespace + "', " + this.builder.expose + ").$2$3";
          } else if (!this.builder.packaging && (this.builder.expose != null)) {
            repl = "$1 " + this.builder.expose + ".$2$3";
          }
          if (repl != null) {
  ...
  pkg('toaster').Toast = Toast = (function() {
    ...
    Toast.prototype.src_folders = null;
    ...
    Toast.prototype.toast = function(srcpath, params) {
      var alias, folder, item, _i, _len, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8;
      if (params == null) {
        params = {};
      }
      if (this.src_folders != null) {
        warn("Can't toast twice simultaneously, link all folders that " + "you need in on config.'.yellow}".yellow);
        return;
      }
      this.src_folders = [];
      ...
      this.vendors = (_ref2 = params.vendors) != null ? _ref2 : [];
      this.exclude = (_ref3 = params.exclude) != null ? _ref3 : [];
      this.bare = (_ref4 = params.bare) != null ? _ref4 : false;
      this.packaging = (_ref5 = params.packaging) != null ? _ref5 : true;
      this.expose = ((_ref6 = params.expose === void 0) != null ? _ref6 : null) ? void 0 : params.expose;
      this.minify = (_ref7 = params.minify) != null ? _ref7 : false;
      this.httpfolder = (_ref8 = params.httpfolder) != null ? _ref8 : "";
  ...
  Toaster = (function() {
    ...
    function Toaster() {
      ...
      if (this.cli.argv.v) {
        ...
        this.builder = new Builder(this, this.toast, this.cli);
      } else {
