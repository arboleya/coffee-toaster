  ...
  Watcher = (function() {
    ...
    Watcher.prototype.watch_folder = function(path) {
      ...
      exec("ls " + path, __bind(function(error, stdout, stderr) {
        return this.snapshots[path] = this.clean_ls(stdout);
      }, this));
      return fs.watchFile(path, {
        interval: 250
      }, __bind(function(curr, prev) {
        return exec("ls " + path, __bind(function(error, stdout, stderr) {
          ...
          diff = this.diff(this.snapshots[path], this.clean_ls(stdout));
          for (_i = 0, _len = diff.length; _i < _len; _i++) {
            item = diff[_i];
            switch (item.action) {
              case "created":
                console.log("New file created, start watching: " + path + "/" + item.path);
                this.watch_file("" + path + "/" + item.path);
                break;
              case "deleted":
    ...
    Watcher.prototype.clean_ls = function(stdout) {
      var index, item, list, _len;
