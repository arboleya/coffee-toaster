(function() {
  var ArrayUtil, Builder, Cli, Config, FsUtil, Module, Project, Question, Script, StringUtil, Toaster, argv, colors, cs, exec, fs, optimist, opts, path, pkg, pn, toaster, uglify, uglify_parser;
  ...
  pkg('toaster.core').Script = Script = (function() {
    ...
    Script.prototype.getinfo = function() {
      var exports, item, klass, repl, requirements, rgx, rgx_ext, _i, _j, _len, _len2, _ref, _ref2, _results;
  ...
  pkg('toaster').Cli = Cli = (function() {
    function Cli() {
      ...
      this.argv = (this.opts = optimist.usage(usage).alias('n', 'new').describe('n', "Scaffold a very basic new App").alias('i', 'init').describe('i', "Create a config (toaster.coffee) file").alias('w', 'watch').describe('w', "Start watching/compiling your project").alias('c', 'compile').boolean('c').describe('c', "Compile the entire project, without watching it.").alias('d', 'debug').boolean('d')["default"]('d', false).describe('d', 'Debug mode (compile js files individually)').alias('b', 'bare').boolean('b')["default"]('b', false).describe('b', 'Compile files with "coffee --bare" (no js wrapper)').alias('p', 'packaging').boolean('p')["default"]('p', false).describe('p', 'Enables/disables the packaging system').alias('m', 'minify').boolean('m')["default"]('m', false).describe('m', 'Minify release code using uglify.').alias('e', 'exports').string('e')["default"]('e', false).describe('e', 'Specify a macro scope to list everything.').alias('v', 'version').describe('v', '').alias('h', 'help').describe('h', '')).argv;
    }
    return Cli;
  })();
  fs = require("fs");
  ...
  pkg('toaster.core').Module = Module = (function() {
    ...
    function Module(toaster, config, opts) {
      var exports, _ref, _ref2, _ref3, _ref4;
      this.toaster = toaster;
      this.config = config;
      this.opts = opts;
      ...
      this.bare = (_ref = this.opts.argv.bare) != null ? _ref : this.config.bare;
      this.exports = (_ref2 = this.opts.argv.exports) != null ? _ref2 : this.config.exports;
      this.packaging = (_ref3 = this.opts.argv.packaging) != null ? _ref3 : this.config.packaging;
      this.minify = (_ref4 = this.opts.argv.minify) != null ? _ref4 : this.config.minify;
    ...
    Module.prototype.compile = function(include_vendors) {
      var compiled, file, line, msg, namespaces, output, _i, _j, _len, _len2, _ref, _ref2;
    ...
    Module.prototype.write = function() {
      var ast, contents, file, filepath, folderpath, index, relative, tmpl, toaster_buffer, toaster_src, vendors, _len, _ref;
      ...
      for (i = 0, _len = _ref.length; i < _len; i++) {
        ...
        _results.push((function() {
          ...
          for (_i = 0, _len3 = _ref3.length; _i < _len3; _i++) {
            ...
            _results2.push(not_found && !this.missing[bc] ? (this.missing[bc] = true, console.log(("" + 'WARNING!'.bold + " Base class").yellow, bc.bold.white, "not found fc class".yellow, file.classname.bold.white, "in file".yellow, file.filepath.yellow)) : void 0);
          }
          return _results2;
        }).call(this));
      }
