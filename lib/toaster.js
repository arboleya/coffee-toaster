(function() {
  ...
  Help = (function() {
    function Help() {
      ...
      adendo = "use w/ [-s]";
      this.argv = (this.opts = optimist.usage(usage).alias('n', 'new').describe('n', "Scaffold a very basic new App, " + adendo).alias('i', 'init').describe('i', "Create a config (toaster.coffee) file, " + adendo).alias('w', 'watch').describe('w', "Start watching/compiling your project, " + adendo).alias('d', 'debug').boolean('d')["default"]('d', false).describe('d', 'Debug mode (compile js files individually)').alias('s', 'script').boolean('s')["default"]('s', false).describe('s', 'Enable CoffeeScript handling').alias('e', 'examples').boolean('e')["default"]('e', false).describe('e', 'Print a list of usage examples, with description').alias('v', 'version').describe('v', '').alias('h', 'help').describe('h', '')).argv;
    }
    return Help;
  })();
}).call(this);
