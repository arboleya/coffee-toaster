(function() {
  var ArrayUtil, Builder, Cli, Config, FsUtil, Module, Project, Question, Script, StringUtil, Toaster, error, growl, icon_error, icon_warn, interval, log, msgs, pkg, process_msgs, queue_msg, start_worker, stop_worker, toaster, warn,
  ...
  pkg('toaster').Config = Config = (function() {
    ...
    Config.prototype.vendor = function(name, src) {
      this.vendors[name] = pn("" + this.basepath + "/" + src);
      return this;
    };
    ...
    Config.prototype.src = function() {
      ...
      this.current_src = path;
      return this;
    };

    Config.prototype.module = function(name, params) {
      params.name = name;
      params.src = pn("" + this.basepath + "/" + this.current_src + "/");
      ...
      for (_i = 0, _len = vendors.length; _i < _len; _i++) {
        ...
        if ((vendor = this.toaster.config.vendors[vendor_name]) != null) {
          if (path.existsSync(vendor)) {
            ...
            log("WARNING!".bold.yellow, "Vendor".white, vendor_name.yellow.bold, "not found at".white, vendor.yellow.bold);
          }
        } else {
          log("WARNING!".bold.yellow, "Vendor".yellow, vendor_name.white.bold, "not found for module".yellow, this.name.white.bold);
        }
      }
      return buffer.join("\n");
    };
